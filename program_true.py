...
#        self.new_graph = reader.load_csv('tau_v2.csv')  # tau_graph가 list가 아니라서 deep copy가 안되므로
    ...
    def is_check_node_available_in_dpt(self, check_node):
        res = True
    ...
    def is_check_node_available_in_arv(self, attachable_path : Path, check_node):
        res = True
        for path in self.DR_node_list:
    ...
    def get_nearest_UAV_path(self, target_node)->Path:
    ...
    def choose_SK_usage(self, check_node):
        ...
        B. check_node가 DR에 포함될 경우 DR의 경로 및 총 거리를 구한다.
            - greedy_path = self.get_greedy_path()
            - DR_path = self.calculate_DR_path([DR_node_lsit... + check_node], greedy_path)
        C. check_node가 UAV에 포함될 경우 UAV의 경로 및 총 거리를 구한다.
            - UAV_path = self.calculate_UAV_path([UAV_node_list... + check_node], greedy_path)
        
        A, B, C 를 비교하여 최종적으로 check_node가 어떤 경로에 포함될 것인지 결정한다.
            1. greedy_path로 truck_dist를 구한다.
            2. bike_a_path로 DR_dist를 구한다.
            3. bike_b_path로 UAV_dist를 구한다.
            1,2,3 값을 비교하여 check_node를 self.DR_node_list, self.UAV_node_list 중 하나에 추가하거나 둘 다 추가하지 않는다.
        '''       
        ...
        if UAV_path != None:
            ...
            if nearest_DR_path != None:
                ...
                if (UAV_path_info_when_UAV2 != None):
                    if UAV_path_info_when_UAV == None or (UAV_path_info_when_UAV2[2] < UAV_path_info_when_UAV[2]):
                ...
                UAV_path = None
        #UAV_node_total_dist = self.get_mixed_dist(greedy_path_when_UAV, UAV_pathes_when_UAV)
        
        self.assign_DR_UAV(DR_path, DR_path_info_when_DR, UAV_path, UAV_path_info_when_UAV)
    ...
    def calculate_DR_path(self, check_node, greedy_path, avail_path):
        ...
        if self.is_check_node_available_in_arv(avail_path, check_node) == False:
            DR_path_info = None
        else:
            DR_path_info = self.choose_dpt_arv(DR_input, check_node, greedy_path, avail_path)

            # 경로를 찾지 못했을 경우에는 False 한다, total_SK_dist를 따진다
            if DR_path_info[2] == 0:
                DR_path_info = None
