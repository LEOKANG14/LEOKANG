...
class Program:
    ...
    def get_available_DR_path(self, target_node):     #DR_path_list 중에 target 노드와 가장 가까운 마지막 노드를 가진 available_DR_path를 출력
        available_DR_path = None     ###질문1###, available_DR_path를 고르는 데에 조건을 추가 len(DR_path.nodes)가 DR.SK_capa=3 이면 선택되지 못하게 
        min_dist = INFINITE
    ...
    def is_check_node_available_in_arv(self, attachable_path : Path, check_node):     #check_node가 arv로 가능한지(True) 출력
        ...
        for path in self.DR_path_list:
            if path == attachable_path:     #어떠한 path가 available_nodes이면 통과 (###질문2###)
                continue
    ...
    def decision_best_path(self, check_node_list):     #input_list가 들어올 때 앞 노드부터 넣어가면서 
        ...
        for check_node in check_node_list:
            if self.is_check_node_available_in_dpt(check_node):     #check_node가 dpt_node로 가능하면(###질문3###)
    ...
    def assign_node(self, path : Path, path_info):     #_path_info에 있는 노드를 path에 배정한다
        ...
        if path == None:     #path가 존재하지 않으면(첫번째 배정이면)
            ...
            path.set_path(nodes[0],nodes[1:-1],nodes[-1])     #nodes를 path에 입력
            if path_info[0] == 'UAV':     ###질문4### 이부분 이상, 아예 UAV에는 배정이 되지 않아야 함
                path.left_parcel = 0
            elif path_info[0] == 'DR':
    ...
    def assign_DR_UAV(self, DR_path, DR_path_info_when_DR, UAV_path, UAV_path_info_when_UAV):     #_path 와 _path_info 정보들을 넣어서 고른다
        ...
        if DR_path_info_when_DR == None and UAV_path_info_when_UAV == None:     #둘다 배정되지 않으면 현재 sequence 취소
            ...
            self.assign_node(UAV_path, UAV_path_info_when_UAV)     #DR의 Total_dist가 UAV의 Total_dist 보다 크면, UAV로 배정
    ...
    def search_available_arv_nodes(self, target_node, dpt_node, SK, greedy_path):     #available_arv_nodes를 찾는다
        '''
        greedy_truck_path 중 dpt_node를 포함하며, bike의 disabled_nodes를 포함하지 않을 수 있는 arv 후보 목록을 찾는다.
        '''
        available_nodes = [ ]    # second_node로 가능한 노드 리스트
        ...
        dpt_index = greedy_path.index(dpt_node)  # dpt_node의 인덱스

        for i in disabled:
            tgt_index = greedy_path.index(i)

            if tgt_index < dpt_index and tgt_index > start_index:
                start_index = tgt_index

            if tgt_index > dpt_index and tgt_index < end_index:
                end_index = tgt_index
        ...
        print("available_arv_path:", available_nodes)
