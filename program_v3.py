...
class Program:
    ...
    def assign_node(self, path : Path, path_info):     #_path_info에 있는 노드를 path에 배정한다
        ...
        if path == None:     #path가 존재하지 않으면(첫번째 배정이면)
            ...
            if path.dpt_idx == nodes[-1]     #dpt와 arv가 뒤집어지면 !!!중요!!!, 원래 path의 interval nodes에 +1 씩 해주고,
                is_reversed = [True, path.dpt_idx, path.arv_idx]
                start = nodes[0]
                end = nodes[-1]
            else:
                is_reversed = [False]
        ...
        return path_info[0], start, end, is_reversed     #-1을 위한 start, end 받음
    ...
    def assign_DR_UAV(self, DR_path, DR_path_info_when_DR, UAV_path, UAV_path_info_when_UAV):     #_path 와 _path_info 정보들을 넣어서 고른다
        if DR_path_info_when_DR == None and UAV_path_info_when_UAV == None:     #둘다 배정되지 않으면 현재 sequence 취소
            ...
            which_SK, start, end, is_reversed = self.assign_node(UAV_path, UAV_path_info_when_UAV)
        elif UAV_path_info_when_UAV == None:     #UAV에 배정되지 않으면, DR로 배정
            which_SK, start, end, is_reversed = self.assign_node(DR_path, DR_path_info_when_DR)
        elif DR_path_info_when_DR[2] <= UAV_path_info_when_UAV[2]:     #DR의 Total_dist가 UAV의 Total_dist 보다 작거나 같으면, DR로 배정
            which_SK, start, end, is_reversed = self.assign_node(DR_path, DR_path_info_when_DR)
        else:
            which_SK, start, end, is_reversed = self.assign_node(UAV_path, UAV_path_info_when_UAV)     #DR의 Total_dist가 UAV의 Total_dist 보다 크면, UAV로 배정

        return  which_SK, start, end, is_reversed
    ...
    def choose_SK_usage(self, check_node):     #check_node를 Truck, DR, UAV중에 하나로 배정한다
        ...
        which_SK, start, end, is_reversed = self.assign_DR_UAV(DR_path, DR_path_info_when_DR, UAV_path, UAV_path_info_when_UAV)
        if which_SK =='DR':
    ...
    def get_dpt_node(self, SK, target_node, greedy_path):     #현재 노드에서 가장 가까운 트럭 노드를 찾는다
        dpt_node = None
        min_dist = INFINITE

        for truck_node_number in greedy_path:
            if truck_node_number in SK.get_disable_nodes():
                continue
    ...
    def search_available_arv_nodes(self, target_node, dpt_node, SK : SK, greedy_path):     #available_arv_nodes를 찾는다
        ...
        available_nodes = [ ]
        '''
        disabled = SK.clone_disable_nodes()     #num_of_SK가 0이 시작되는 노드(좌우)
        ...
        right_gap = end_index - dpt_index
        right_index = dpt_index
        for _ in range(len(right_gap)):
            right_index = dpt_index + 1
            if greedy_path.index(right_index) = 0:
                SK.set_disable_nodes(greedy_path.index(right_index))

        left_gap = dpt_index - start_index
        left_index = dpt_index
        for _ in range(len(left_gap)):
            left_index = dpt_index - 1
            if greedy_path.index(left_index) = 0:
                SK.set_disable_nodes(greedy_path.index(left_index))
        '''
        dpt_index = greedy_path.index(dpt_node)     # 현재 입력된 dpt_node의 인덱스

        right_nodes = greedy_path[dpt_index+1:]     # 뒤로 쭉 다 
